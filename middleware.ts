...
const authenticateUser = async (token: string | undefined, request: NextRequest) => {
  ...
  if (!token) {
    ...
    return NextResponse.redirect(new URL(`/login?msg=${encodeURIComponent('You must be logged in to access this!')}`, request.url));
  }
  ...
  if (!user) {
    ...
    return NextResponse.redirect(new URL(`/login?msg=${encodeURIComponent('Your session is expired!')}`, request.url));
  }
  return user;
}
...
export const middleware = async (request: NextRequest) => {
  ...
  switch (request.nextUrl.pathname) {
    case '/admin':
      ...
      break;
    case '/add-news': {
      const res = await authenticateUser(token, request) as IUser;
      if (res instanceof (NextResponse)) {
        return res;
      }
